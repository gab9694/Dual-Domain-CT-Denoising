...
from cal_ssim import psnr, ssim
    # ori_psnr = compute_PSNR(x, y, data_range)
    # ori_ssim = compute_SSIM(x, y, data_range)
    # ori_result = []
    # ori_result.append((ori_psnr, ori_ssim))
    # ori_result = np.array(ori_result)   

def compute_measure_simple(x, y, pred, trunc_min, trunc_max):
    data_range = trunc_max - trunc_min
    pred_b = pred.unsqueeze(0).unsqueeze(0) - trunc_min
    y_b = y.unsqueeze(0).unsqueeze(0) - trunc_min
    cal_pnsr_v = psnr(pred_b, y_b, data_range=data_range)
    # pred_ssim = compute_SSIM(pred, y, data_range)
    cal_ssim_v = ssim(pred_b, y_b, data_range=data_range)
    pred_psnr = cal_pnsr_v.item()
    pred_ssim = cal_ssim_v.item()
    pred_result = []
